...
MofaflexR provides an R-native bridge between [SingleCellExperiment](https://bioconductor.org/packages/SingleCellExperiment/) objects and the Python [MOFAFlex](https://github.com/bioFAM/mofaflex) package. The key contribution is a **zero-copy sparse bridge**: count matrices stored as `Matrix::dgCMatrix` are exposed to Python `scipy.sparse` without densifying or allocating a second copy of the data.
