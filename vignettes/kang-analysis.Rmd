...
3. **Bridge** the resulting `SingleCellExperiment` into a Python `AnnData`
   via `sce_to_reticulate_anndata()`.
...
`sce_to_reticulate_anndata()` builds a Python `AnnData` from the SCE without
copying the underlying sparse matrix (zero-copy bridge). All `colData` columns
are forwarded to `adata.obs`.
